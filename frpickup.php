...
?>
	...
	<h3 class="titlehdr">Create a New Food Rescue Pickup for <?php echo $_SESSION['Username']?></h3>
		<?php
		...
		<form name="OrderForm" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
			...
			if (checkAdmin()){
				$result = mysql_query("SELECT DonorNumber, DonorName, Address1 FROM Donor WHERE FoodRescue = 'Yes'");
			} else {
				$result = mysql_query("SELECT DonorNumber, DonorName, Address1 FROM Donor WHERE FoodRescue = 'Yes' AND ProgramNumber = '$_SESSION[Username]'");
