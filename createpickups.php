...
if (array_key_exists('Submit',$_POST)) {
	...
	if (empty($errors)) {
		...
		if($BreadQty == 0 && $AssortedRefrigeratedProductQty == 0 && $ProduceQty == 0  && $AssortedMixedDryQty == 0 && $AssortedFrozenQty == 0 ) {
			die("You cannot submit a pickup with 0 Quantity. Please go <a href='createpickups.php'>back</a> and try again.");
		}
...
?>
	...
	<h3 class="titlehdr">Create a New Pickup</h3>
		...
		<form name="CreatePickup" method="post" action="createpickups.php">
			<div class="cf">
			<div class="itemlabels"><label for="PickupDate" class="bolded">Pickup Date <sup>*</sup></label></div>
			<div class="rightcolumn"><input name="PickupDate" type="text" id="PickupDate" size="8" maxlength="1" required <?php if (isset($errors)) { echo 'value="'.htmlentities($_POST['PickupDate']).'"'; } ?>></div><?php if (isset($errors) && in_array('PickupDate', $errors)){?><div class="red">Required</div><?php } ?>
			</div>
			...
			<div class="rightcolumn">
			<select name="DonorNumber" class="DonorNumber" required>
	...
	<!-- ajax for donor selection based off selected agency -->
	<script type="text/javascript">
	$(document).ready(function()
	{
	$(".ProgramNumber").change(function()
	{
	var ProgramNumber=$(this).val();
	var dataString = 'ProgramNumber='+ ProgramNumber;
