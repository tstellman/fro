...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	...
	<table align="center">
		<?php loadChart($type,$startDate,$endDate); ?>
	</table>
	<table align="center">
		<thead>
			<tr>
...
if ($type == "Donor"){		
		...
		$chartResult = mysql_query("SELECT ph.PickupId, p.ProgramNumber, p.ProgramName, pt.SUMTotalAmount, pt.DonorNumber
										...
										FROM PickupHeader ph
										WHERE (ph.PickupDate BETWEEN '".$_POST['startDate']."' AND '".$_POST['endDate']."')" . $initAgencyNo . $AgencyNo . $DonorNo . $PickupType ."
		...
		if ($chartResult) {
			...
			while($cRow = mysql_fetch_array($chartResult)) {
				array_push($arrData["data"], array(
					...
					"value" => $cRow['SUMTotalAmount'],
					"link" => "programreport.php?t=Donor&sd=" . $_POST['startDate'] . "&ed=" . $_POST['endDate']
					)
				);
			}
